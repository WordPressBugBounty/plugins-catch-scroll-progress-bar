					...
					<div class="option-container">
						<table class="form-table" bgcolor="white">
							<tbody>
									...
									<td>
										<select name="catch_progress_bar_options[progress_bar_position]" id="catch_progress_bar_options[progress_bar_position]">
											...
											foreach ($progress_bar_position as $k => $value) {
												echo '<option value="' . esc_attr($k) . '"' . selected($settings['progress_bar_position'], $k, false) . '>' . esc_html($value) . '</option>';
											}
											?>
										</select>
								...
								<tr>
									<th>
										<label><?php esc_html_e('Border Radius', 'catch-scroll-progress-bar'); ?></label>
									</th>
									<td>
								...
								<tr>
									<th>
										<?php echo esc_html__('Select the Template Condition to progress bar', 'catch-scroll-progress-bar'); ?>
									</th>
									<td>
								...
								<tr>
									<th>
										<?php echo esc_html__('Select the Post type to  apply progress bar', 'catch-scroll-progress-bar'); ?>
									</th>
									<td>
										...
										<div>
											...
											if (isset($settings['field_posttypes'])) {
												...
												foreach ($post_types as $type => $obj) {
													...
													if (isset($optionPostTypes[$obj->name])) : $optionNamePostType = $optionPostTypes[$obj->name];
													else : $optionNamePostType = '';
													endif;
											?>
													<p><input type='checkbox' name='catch_progress_bar_options[field_posttypes][<?php echo esc_attr($obj->name); ?>]' id="catch_progress_bar_options[field_posttypes][<?php echo esc_attr($obj->name); ?>]"
															<?php echo checked(isset($optionNamePostType) ? $optionNamePostType : '', true, false); ?> value='1' /> <?php echo esc_html($obj->labels->name); ?></p>
												<?php
												}
											} else {
												$post_types = get_post_types(array('public' => true), 'objects');
												foreach ($post_types as $type => $obj) {
												?>
													<p><input type='checkbox' name='catch_progress_bar_options[field_posttypes][<?php echo esc_attr($obj->name); ?>]' value='1' /> <?php echo esc_html($obj->labels->name); ?></p>
											<?php
												}
											} ?>
